...
begin reaction rules
    ...
    NAD() + LDH_PYR() <-> LDH_PYR_NAD() kf, kr
    NADH() + LDH_LAC() <-> LDH_LAC_NADH() kf, kr
    LAC() + LDH_NADH() <-> LDH_LAC_NADH() kf, kr
    PYR() + LDH_NAD() <-> LDH_PYR_NAD() kf, kr
...
# Write BNGL file
with open("assign-100.bngl", "w") as f:
    f.write(bngl_model)

# Run BNGL with BioNetGen
subprocess.run(["bionetgen", "run", "-i", "assign-100.bngl"], 
               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, 
               check=True)

# Load simulation results
res = pd.read_csv("assign-100.gdat", sep=r'\s+', comment="#", header=None)
